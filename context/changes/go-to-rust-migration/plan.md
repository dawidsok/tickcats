...
| TUI-05 | Full-screen detail view with markdown highlighting, metadata, scrolling, and identity preservation (`internal/tui/render_detail.go`; `movement.go`) | Shipped | None | Replace — show read-only highlighted Markdown/metadata in a scrollable side panel while the board remains visible (user, 2026-08-06) | Enter/`o` opens the selected ticket panel, `e` edits externally, Esc closes it, and reload/move/delete preserves or reports identity. |
