...
| TUI-19 | Six themes with deterministic dynamic-column gradient (`internal/tui/model.go:79-99`; `color_test.go`) | Shipped | `config.theme` | Replace — one fixed Rust palette; preserve but ignore legacy theme value (user, 2026-08-06) | No theme selector/gradient engine exists; the fixed four-column palette passes readability and focus-state checks. |
