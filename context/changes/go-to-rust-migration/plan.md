...
| CLI-04 | `new feat\|feature\|task\|bug\|fix`, P2 default, generated ID, backlog destination (`cmd/tickcats/main.go:79-100`; `internal/store/create.go`) | Shipped; aliases undocumented | New ticket | Replace — accept only documented `feat\|task\|bug`; retain generated ID, P2, backlog, and printed path (user, 2026-08-06) | Process tests accept three kinds, reject `feature`/`fix`, and verify exact created ticket/path behavior. |
