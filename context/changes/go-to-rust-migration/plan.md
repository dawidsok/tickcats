...
| CLI-07 | `move <ticket> <from> <to>` accepts IDs/display names, validates paths, and refuses collisions (`cmd/tickcats/main.go:124-148`) | Shipped | Renames file | Review — user | Golden cases cover aliases, traversal rejection, invalid source, and collision. |
...
| CLI-10 | `ids migrate` rewrites IDs and filenames and prints mappings (`cmd/tickcats/main.go:184-197`) | Shipped | Many tickets | Review — user | Approved legacy/custom-column and collision behavior has fixture tests. |
...
| TUI-09 | `p` progress and `b` move back one column (`internal/tui/update.go:124-136`) | Shipped; `b` undocumented in README | Renames ticket | Review — user | First/middle/last-column behavior is tested. |
| TUI-10 | Move mode `m`, `h/l`, `H/L`, and cancel (`internal/tui/update.go:199-223`) | Shipped | Renames tickets | Review — user | Approved single-ticket movement preserves focus and selection. |
...
| TUI-14 | Deadline dialog: today, tomorrow, +7 days, custom date, clear (`internal/tui/deadline_dialog.go`) | Shipped | Ticket metadata | Review — user | Board/detail branches and invalid-date handling pass. |
| TUI-15 | `i` important toggle and matrix-aware priority display/order (`internal/tui/actions.go:77-97`; `sort.go`) | Shipped | Ticket/config | Review — user | Toggle, urgent threshold, buckets, and P-label suppression pass. |
| TUI-16 | `s` cycles priority → title → date → deadline → manual (`internal/tui/actions.go:172-191`) | Shipped | `sort.json` | Review — user | Exact approved cycle and tie-break ordering pass. |
