...
| TUI-05 | Full-screen detail view with markdown highlighting, metadata, scrolling, and identity preservation (`internal/tui/render_detail.go`; `movement.go`) | Shipped | None | Replace — scrollable side panel when wide, full-screen detail fallback when narrow (user, 2026-08-06) | Enter/`o`, `e`, Esc, scrolling, breakpoint behavior, and reload/move/delete identity are covered at both layout sizes. |
