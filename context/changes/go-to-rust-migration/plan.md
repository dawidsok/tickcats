...
| CLI-03 | `init` creates defaults and idempotently adds `<board-basename>/` to adjacent `.gitignore` (`internal/store/init.go`) | Shipped | Folders, `.gitignore` | Retain — create four fixed folders and ignore the selected board path (user, 2026-08-06) | Temp fixtures cover first/repeat run, existing board, custom path, and exact Backlog/Ready/`doing`/Done folders. |
