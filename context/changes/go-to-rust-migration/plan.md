...
| CLI-02 | Global `--path <dir>` may appear before dispatch and selects board root (`cmd/tickcats/main.go:28-45`) | Shipped | Selects all files | Retain — every command/TUI launch accepts an explicit board directory (user, 2026-08-06) | Before/after-subcommand placement, missing value, custom basename, and default path have process tests. |
| CLI-03 | `init` creates defaults and idempotently adds `<board-basename>/` to adjacent `.gitignore` (`internal/store/init.go`) | Shipped | Folders, `.gitignore` | Retain — create four fixed folders, ignore the board path, and support `--no-intro` for automation (user, 2026-08-06) | Temp fixtures cover first/repeat run, custom path, exact folders, default intro creation, and `--no-intro`. |
| CLI-04 | `new feat\|feature\|task\|bug\|fix`, P2 default, generated ID, backlog destination (`cmd/tickcats/main.go:79-100`; `internal/store/create.go`) | Shipped; aliases undocumented | New ticket | Replace — accept only `feat\|task\|bug` and default every ticket to `[to refine]`; retain ID, P2, backlog, and printed path (user, 2026-08-06) | Process tests verify three kinds, rejected aliases, canonical labeled title, metadata/body, and exact printed path. |
...
| CLI-07 | `move <ticket> <from> <to>` accepts IDs/display names, validates paths, and refuses collisions (`cmd/tickcats/main.go:124-148`) | Shipped | Renames file | Replace — adjacent Backlog↔Ready↔WIP↔Done; `wip` is canonical and legacy `doing` remains an input alias (user, 2026-08-06) | Output/help use `wip`; cases accept both aliases, enforce adjacent movement, and reject skipped columns without mutation. |
...
| CLI-14 | Introductory instruction ticket on initialization | Approved addition | New backlog ticket | Retain — first new board gets one guided ordinary ticket unless `init --no-intro`; never recreate/protect it (user, 2026-08-06) | Fixture asserts content/behavior, repeat init creates none, and explicit automation opt-out creates no tutorial file. |

### TUI workflows

| ID | Current feature and evidence | Baseline | Data impact | Decision / owner | Rust acceptance check |
|---|---|---|---|---|---|
| TUI-01 | Board shows pick-next banner, configured columns, focused ticket, status/warnings, and footer (`internal/tui/render_board.go`) | Shipped | None | Replace — compact two-line cards and no top banner; mark every CORE-03 recommendation and show “choose one” for exact ties (user, 2026-08-06) | Card line 1 is title; line 2 is ID plus priority/matrix and read-only deadline/urgency. Markers are non-color/distinct from focus, and matrix position never changes recommendation. |
...
| TUI-05 | Full-screen detail view with markdown highlighting, metadata, scrolling, and identity preservation (`internal/tui/render_detail.go`; `movement.go`) | Shipped | None | Replace — right-side ~40% scrollable panel when wide, full-screen detail fallback when narrow (user, 2026-08-06) | Board remains on the left with stable focus; Enter/`o`, `e`, Esc, scrolling, breakpoint, and reload/move/delete identity are covered at both sizes. |
| TUI-06 | New-ticket form: kind, title, P0-P3, default P2, default `[to refine]` (`internal/tui/create.go`) | Shipped; differs from CLI | Creates ticket | Retain — keep all four fields and current Feature/P2/to-refine defaults (user, 2026-08-06) | Field navigation, empty-title validation, kind/priority choices, label toggle, and backlog output pass. |
...
| TUI-16 | `s` cycles priority → title → date → deadline → manual (`internal/tui/actions.go:172-191`) | Shipped | `sort.json` | Drop — board order is fixed (user, 2026-08-06) | Rust TUI has no sort key, mode indicator, sort config mutation, or reorder prompt. |
| TUI-17 | `/` fuzzy subsequence search over priority/title/body with typing/navigation phases and two-step Esc (`internal/tui/search.go`) | Shipped; partly undocumented | None | Retain — current fuzzy scope and typing/navigation model (user, 2026-08-06) | Query, match count, priority/title/body matching, cross-column navigation, side-panel open, edit mode, clear, and exit pass. |
| TUI-18 | Config editor presets/custom command (`internal/tui/config_view.go`) | Shipped | `config.editor` | Drop — editor is controlled by `$EDITOR`, not board config (user, 2026-08-06) | Rust exposes no editor preset/custom setting and never erases the legacy config value. |
| TUI-19 | Six themes with deterministic dynamic-column gradient (`internal/tui/model.go:79-99`; `color_test.go`) | Shipped | `config.theme` | Replace — one fixed neutral palette with semantic color only for focus, urgency, warnings, and success/error (user, 2026-08-06) | No theme selector/gradient engine exists; meaning is not conveyed by color alone and focus/readability checks pass. |
| TUI-20 | Config matrix on/off (`internal/tui/config_view.go`) | Shipped | `disable_matrix_prioritisation` | Replace — remove config view and toggle matrix directly with board hotkey `M` (user, 2026-08-06) | `M` persists the toggle without erasing legacy config fields and immediately reapplies ordering/display. |
...
| TUI-22 | Column color picker/rendering described in root `plan.md`; storage only is committed | In flight | `columns[].color` | Drop — no per-column color customisation or picker in Rust (user, 2026-08-06) | Rust contains no color palette/picker flow; fixed-column styling is decided separately from theme presets. |
| TUI-23 | Scrollable help overlay and mode-specific key reference (`internal/tui/help_dialog.go`) | Shipped | None | Retain — `?` opens reduced, mode-specific key help; `?`, Enter, or Esc closes (user, 2026-08-06) | Open/scroll/close tests and content snapshots contain only retained keys/features. |
| TUI-24 | Quit confirmation from board/detail/move; Ctrl-C immediate (`internal/tui/update.go:18-70`) | Shipped | None | Replace — `q` and Ctrl-C restore terminal and quit immediately from every non-text-entry mode (user, 2026-08-06) | No quit overlay/state exists; text fields consume ordinary `q`, while Ctrl-C always exits safely. |
| TUI-25 | Manual reload plus debounced external file watcher; focus/detail/selection reconciliation (`internal/tui/watcher.go`; `actions.go`) | Shipped | Reads board | Replace — keep `r` and post-editor reload; drop automatic file watching (user, 2026-08-06) | Manual/editor reload preserves focus and open detail identity; no watcher dependency, task, debounce, or event lifecycle exists. |
| TUI-26 | Three-second generation-safe success/error/info notifications (`internal/tui/model.go:199-224`) | Shipped; undocumented | None | Replace — persistent status line plus one context-sensitive common-key hint line; `?` holds full help (user, 2026-08-06) | Footer consumes two rows, does not wrap at supported widths, uses non-color-redundant status styling, and has no timer/generation state. |
...
| OPS-08 | Bundled `tc-*` and roadmap Agent Skills invoke exact CLI/path/folder contracts (`skills/`) | Repo tooling | May mutate boards/gitignore | Retain and update — fixed columns, adjacent moves, CLI `[to refine]`, `--print-path`, and `init --no-intro` are supported contracts (user, 2026-08-06) | Representative shape→ticket→plan→implement workflow creates no tutorial ticket and has no stale Go/custom-column assumptions. |
...
**Contract**: `__complete tickets` emits files from the four fixed folders; `__complete columns` emits `backlog`, `ready`, `wip`, `done`. CLI resolution maps canonical `wip` and legacy alias `doing` to `doing/`; completions exclude legacy folders and write nothing.
...
- Filesystem snapshots pass for first/repeat/`--no-intro` init and intro-ticket non-regeneration, create, adjacent move, important mutation, fixed-folder warnings, collisions, malformed files, and ID migration preflight/partial-report/idempotent rerun.
...
- [x] 1.1 Go baseline tests pass. — 91d8f6a
- [x] 1.2 Rust format, lint, and skeleton tests pass. — 91d8f6a
- [x] 1.3 Fixture manifest covers all approved feature IDs and DATA rows. — 91d8f6a
- [x] 1.4 Process harness captures streams, status, and copied-tree side effects. — 91d8f6a

#### Manual

- [x] 1.5 Rust modules/dependencies contain nothing for dropped/deferred features. — 91d8f6a
- [x] 1.6 Fixture expectations match approved intent. — 91d8f6a
...
- [x] 3.1 Retained CORE/CLI mappings pass Rust tests. — 3729480
- [x] 3.2 Process contracts pass for all retained commands and errors. — 3729480
- [x] 3.3 Filesystem side-effect snapshots pass. — 3729480
- [x] 3.4 Full Go suite still passes. — 3729480
