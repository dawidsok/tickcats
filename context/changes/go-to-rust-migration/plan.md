...
| TUI-08 | External editor command: config, `$EDITOR`, then `vi`; whitespace-split args; board reload (`internal/tui/editor.go`; `actions.go`) | Shipped | Ticket/config | Replace — use `$EDITOR` only with `vi` fallback; preserve but ignore legacy editor config (user, 2026-08-06) | `e` launches the environment-selected editor for the ticket, restores the terminal, and reloads the board on exit. |
