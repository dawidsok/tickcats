...
| CORE-09 | Optional deadline/SLA metadata and mutation (`internal/store/deadline.go`) | Shipped; post-PRD | `deadline`, `updated` | Preserve-data-only — no Rust deadline interaction, urgency styling, or deadline sort (user, 2026-08-06) | Existing `deadline` values survive every ticket rewrite but are exposed only in raw markdown. |
