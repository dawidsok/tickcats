...
name: tc-shape
description: >
  Tickcats-pipeline variant of 10x-shape. Facilitate a structured discovery
  conversation that turns an idea — greenfield or brownfield, auto-detected
  from cwd — into shape-notes.md, the input to /tc-prd. Use when the user
  wants the tc-* (tickcats ticket-based) workflow: "tc shape", "shape with
  tickets", "new tickcats project", "shape an idea for the tickcats board".
  Use BEFORE /tc-prd, not in place of it. For the classic roadmap-file
  workflow use /10x-shape instead.
...
# Shape: Facilitate Discovery (Greenfield & Brownfield) Before /tc-prd

This skill is the head of the tc-* (tickcats) chain: `/tc-shape → /tc-prd → /tc-decompose → /tc-plan → /tc-implement`, greenfield and brownfield alike. Its single job: walk a user from "I have an idea" (greenfield) or "I want to change this system" (brownfield) to a structured `context/foundation/shape-notes.md` that `/tc-prd` can turn into a PRD that conforms to the locked schema.
...
The locked schema both this skill and `/tc-prd` conform to lives at `references/prd-schema.md` (relative to this SKILL.md). Read it before producing any artifact and re-check against it at every checkpoint write.
...
**Skip when**: the project already has a PRD or an ADR set (use `/tc-refine` or `/tc-plan` instead), or the user is reasoning about a single bug / refactor / small feature within an existing codebase that doesn't warrant a full PRD (use `/tc-refine`). For brownfield projects where the user wants to shape a meaningful change, this skill IS the right starting point.

## Relationship to other skills

- `/10x-init` — scaffolds the `/context` skeleton (`changes/`, `archive/`, `foundation/`) plus universal READMEs in each. `/tc-shape` requires `context/foundation/` to exist; if absent, it delegates to `/10x-init` via the `Skill` tool (Step 0 below).
- `/tc-prd` — consumes `shape-notes.md`. The handoff is the `## Step 8` clipboard write.
- `/tc-refine` — for *reframing* small-scope problems within existing systems where a full PRD is overkill. `/tc-shape` is for larger brownfield changes (new modules, significant features) that need structured discovery and a PRD.
- `/tc-decompose` — downstream of `/tc-prd`. Turns the PRD into tickcats ticket stubs (foundations + slices).
- `/tc-plan` — downstream of `/tc-decompose`, never invoked from here directly.

## Initial Response

When this skill is invoked:

1. **If a freeform idea was provided as the argument** (e.g. `/tc-shape a recipe app that suggests meals from what's in your fridge`), capture it verbatim as the **seed idea**. Do not rephrase. Proceed to Step 0.
2. **If a file path was provided** (e.g. `/tc-shape @notes/idea.md`), read it FULLY and use its contents as the seed. Proceed to Step 0.
3. **If nothing was provided**, respond with:

```
I'll help you shape an idea into structured notes that /tc-prd can turn into
...
Tip: pass the idea inline — `/tc-shape a recipe app that uses fridge contents`
     or for brownfield — `/tc-shape add a recommendation engine to my recipe app`
```
...
On "Yes": invoke `/10x-init` via the **Skill** tool (NOT via Bash). When `/10x-init` returns, re-check the precondition; if it now passes, continue to Step 0.5. On "No": print "Stopping. Run `/10x-init` when ready, then re-invoke `/tc-shape`." and STOP.
...
Write the confirmed `context_type` into the shape-notes.md frontmatter (alongside `checkpoint:`) immediately. This value is load-bearing for `/tc-prd`'s auto-routing.
...
- NEVER pre-commit to a stack (framework, database, hosting platform, language family). The PRD captures product-level priors only — `product_type`, `target_scale`, `timeline_budget`. Stack-shaped concerns are gathered downstream of `/tc-prd`.
...
- question: "How would you like to proceed?"
  ...
  - label: "Accept and finish"
    description: "Proceed despite the gaps. They will be recorded as warnings in the checkpoint and surfaced in /tc-prd's Open Questions."
...
```bash
echo -n "/tc-prd" | pbcopy 2>/dev/null || echo -n "/tc-prd" | clip.exe 2>/dev/null || echo -n "/tc-prd" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/tc-prd"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Next:   /tc-prd  (✓ copied to clipboard)

  After /tc-prd, the next chain step will pick up:
...
STOP. Do not chain into `/tc-prd` automatically — the user runs it when ready.
...
5. **Soft gate, not hard gate.** The closing cross-check WARNS but allows the user to override every gap. Override paths are recorded in the checkpoint as `quality_check_status: warned` and surfaced in `/tc-prd`'s `## Open Questions`. Refusing to finish is not in scope.

6. **Mode-aware behavior.** The skill auto-detects context type (greenfield vs brownfield) from project markers in cwd and adapts all six discovery phases accordingly. For brownfield, the discovery loop shifts from "what are you building from scratch?" to "what exists, what's changing, what must be preserved?". If the user invokes this skill for a small-scope problem within an existing codebase (single bug, quick refactor), suggest `/tc-refine` instead — `/tc-shape` is for changes that warrant a full PRD.
...
- This is a **shaping** skill. Output is `shape-notes.md`, not `prd.md`. `/tc-prd` is the document generator.
