...
name: tc-prd
description: >
  Tickcats-pipeline variant of 10x-prd. Generate context/foundation/prd.md
  from shape-notes.md (or raw notes) against the locked PRD schema.
  Auto-routes to greenfield (10 sections) or brownfield (11 sections)
  template based on context_type in shape-notes.md or cwd auto-detection.
  Use when the user wants the tc-* (tickcats ticket-based) workflow:
  "tc prd", "PRD for the tickcats board", "write the PRD then tickets".
  Use AFTER /tc-shape, BEFORE /tc-decompose. For the classic roadmap-file
  workflow use /10x-prd instead.
...
This skill is the second link in the tc-* (tickcats) chain: `/tc-shape → /tc-prd → /tc-decompose → /tc-plan → /tc-implement`, greenfield and brownfield alike. Its single job: take a shaped notes file and emit a `context/foundation/prd.md` that conforms to the locked PRD schema, routing every gap to `## Open Questions` rather than inventing content.
...
The locked schema this skill conforms to lives at `../tc-shape/references/prd-schema.md` (relative to this SKILL.md). Read it before generating any artifact and re-check the produced file against it before writing to disk.

## When to use, when to skip

**Use when**: the user has run `/tc-shape` (and `context/foundation/shape-notes.md` exists with a checkpoint block), OR the user has a raw notes file they want turned into a PRD draft, OR the user explicitly asks to (re-)generate `context/foundation/prd.md`.

**Skip when**: the user is still ideating and has no notes — point at `/tc-shape` first. Skip also when the user wants to *edit* an existing PRD by hand — this skill writes whole files; surgical edits are out of scope.

## Relationship to other skills

- `/tc-shape` — produces `shape-notes.md`, the canonical input. Always preferred upstream of this skill.
- `/tc-decompose` — downstream consumer of `prd.md` (greenfield and brownfield). Decomposes it into tickcats ticket stubs: foundations as `task` tickets, vertical slices as `feat` tickets, with priorities and prerequisite wiring.
- `/tc-refine`, `/tc-plan` — unrelated; PRD is a foundation artifact, not a per-change plan.

## Initial Response

When this skill is invoked:

1. **If a path argument was provided** (e.g. `/tc-prd @notes/raw.md` or `/tc-prd context/foundation/shape-notes.md`), capture it as the input path. Proceed to Step 1.
...
- question: "No input file found at `<resolved-path>`. How would you like to proceed?"
  header: "Input?"
  options:
  - label: "Run /tc-shape first (Recommended)"
    description: "Stop here. Run /tc-shape to produce shape-notes.md, then re-invoke /tc-prd."
...
On "Run /tc-shape first": print "Stopping. Run `/tc-shape` to produce shape-notes.md, then re-invoke `/tc-prd`." and STOP.
...
2. **If no `context_type:` in frontmatter** (raw notes, pasted input) — auto-detect from cwd:
   ...
   Use the same multi-signal detection as `/tc-shape` (Step 0.7): check for git history (Tier 1), lockfiles (Tier 2), manifest files (Tier 3), and bonus signals (source dirs, framework configs). Any Tier 1 or Tier 2 hit → propose brownfield. Tier 3 only → propose brownfield with ambiguity flag. No signals → propose greenfield.
...
1. **Frontmatter `checkpoint:` block present** — strongest signal that this came from `/tc-shape`. Look for the literal `checkpoint:` key inside a YAML frontmatter fence at the top of the file.
...
1. **Frontmatter `checkpoint:` block present AND `context_type: brownfield`** — strongest signal that this came from `/tc-shape` in brownfield mode. Also check for `## Current System` section in the body.
...
time to run /tc-shape first, the resulting PRD will be substantially stronger.
```
...
- question: "How would you like to proceed?"
  ...
  options:
  - label: "Run /tc-shape first (Recommended)"
    description: "Stop here. Use /tc-shape to fill in the missing signals, then re-invoke /tc-prd."
...
On "Run /tc-shape first": print the redirect message and STOP. On "Proceed anyway": continue to Step 3 with `score < 2` recorded so later steps know to expect TODOs. On "Cancel": STOP.

### Step 3: Generate PRD

Read the schema reference FULLY one more time (`../tc-shape/references/prd-schema.md`) to confirm the field list and section names have not drifted.
...
If `/tc-shape` recorded Socrates blockquotes under FRs, preserve them verbatim — they're load-bearing for downstream review.

If shape-notes.md carried a `## Quality cross-check` block (from Step 7 of `/tc-shape`), mirror each gap into `## Open Questions` as a numbered entry naming the missing element and its consequence.
...
have drifted — re-read ../tc-shape/references/prd-schema.md and reconcile.
...
Then copy the next-step command to clipboard and announce (same for greenfield and brownfield):

```bash
echo -n "/tc-decompose" | pbcopy 2>/dev/null || echo -n "/tc-decompose" | clip.exe 2>/dev/null || echo -n "/tc-decompose" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/tc-decompose"
```

```
► Next:   /tc-decompose  (✓ copied to clipboard)

          It decomposes this PRD into tickcats ticket stubs:
          foundations as task tickets, vertical slices as feat
          tickets, with priorities and prerequisite wiring.
          Refine and plan them with /tc-refine and /tc-plan.
```
...
2. **Schema is the contract.** `../tc-shape/references/prd-schema.md` defines frontmatter keys, section names, and section order. Re-read it at every invocation. Re-validate the in-memory PRD against it in Step 3c before writing. Drift between this skill and the schema is the failure mode this skill exists to prevent.
...
7. **Never chain automatically.** The hand-off is an announcement, not an invocation. The user picks when (and whether) to run the next step (`/tc-decompose`). Auto-chaining would skip the human's review of the generated PRD.

## Notes

- This is a **document generator** skill. Output is `context/foundation/prd.md` (or `prd-vN.md`), period.
- The schema reference (`../tc-shape/references/prd-schema.md`) is the single source of truth. Any field name, section name, or frontmatter key referenced in this body MUST exist in the schema doc — if it doesn't, fix the schema doc first.
...
- The `# TODO: <field-name> — see Open Questions` pattern is load-bearing. Downstream tooling (review skills, tc-decompose) can grep for `^# TODO: ` to count unresolved gaps and decide whether the PRD is review-ready.
